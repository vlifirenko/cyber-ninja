...
namespace ToolExtensions
{
    ...
    public class ReplaceObjects : EditorWindow
    {
        ...
        // Add menu item 
        [MenuItem("Tools/Productivity Toolkit/Replace Objects")]
        public static void ShowWindow()
        {
            ReplaceObjects window = (ReplaceObjects)GetWindow(typeof(ReplaceObjects));
        ...
        private void OnGUI()
        {
            ...
            _ReplacementObject = (GameObject)EditorGUILayout.ObjectField("Replacement object",_ReplacementObject, typeof(GameObject), true);
        ...
        private void Replace()
        {
            ...
            foreach (var te in _transformElementsToReplace)
            {
                ...
                if (assetType == PrefabAssetType.NotAPrefab)
                {
                    if (hasparent)
                    {
                        instantiatedObject = Instantiate(_ReplacementObject, te.TheGameObject.transform.position, Quaternion.identity, te.TheGameObject.transform.parent);
                    }
                    else
                    {
                        instantiatedObject = Instantiate(_ReplacementObject, te.TheGameObject.transform.position, Quaternion.identity);
                ...
                else
                {
                    if (hasparent)
                    {
                        var replacementObjectPrefabPath = PrefabUtility.GetPrefabAssetPathOfNearestInstanceRoot(_ReplacementObject);
                        GameObject replacementObjectPrefab = (GameObject)AssetDatabase.LoadAssetAtPath(replacementObjectPrefabPath, typeof(GameObject));
                        instantiatedObject = (GameObject)PrefabUtility.InstantiatePrefab(replacementObjectPrefab);
                        instantiatedObject.transform.parent = te.TheGameObject.transform.parent;
